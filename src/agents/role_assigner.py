...
def _count_since_last_event(history: List[str]) -> int:
    """Count messages since the last Event."""
    cnt = 0
...
def _choose_alternate(champions: List[str], last_champ: Optional[str]) -> str:
    """Choose an alternate champion from the last one."""
    if not champions:
        return "Event"
...
class RoleAssignerAgent(Agent):
    """Agent that determines which champion or event should speak next."""
    
    def __init__(self, role_name: Role, champions_list: List[str] = None):
        self.champions_list = champions_list or []
    ...
    def __call__(self, state: AgentState) -> AgentState:
        ...
        # 2) Force Event after 4 champion lines since last Event
