...
class EventCreatorAgent(Agent):
    """Agent that creates story events based on the scenario and champions."""
    
    def __init__(self, role_name: Role, champion_dict: List[dict], scenario: str):
        self.champion_dict = champion_dict
        self.scenario = scenario
        super().__init__(role_name)

    def _init_system_message(self) -> SystemMessage:
        """Returns the system prompt for event creation."""
        prompt = """
    ...
    def _init_human_message(self) -> HumanMessage:
        """Returns the human message with the scenario."""
        prompt = """
    ...
    def __call__(self, state: AgentState) -> AgentState:
        """Invoke call and parse events into a queue."""
