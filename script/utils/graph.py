...
class StoryTeller():
    def __init__(self, scenario: str, input_json: List):
    ...
    def _preprocess_input(self, input_json):
        # for champ_dict in input_json:
            # print(champ_dict['models'])
            # print(ModelChoices[champ_dict['models'][0]])
        self.champion_dict = {champ_dict['name']: (champ_dict['personality'], ModelChoices[champ_dict['models']].value) for champ_dict in input_json}
    ...
    def build_graph(self):
        ...
        self.graph.add_conditional_edges("RoleAssignerBot", role_assigner_node, list(self.champion_dict.keys()) + ["NovelWriterBot", "RoleAssignerBot"])
    ...
    def invoke(self):
        return self.app.invoke(AgentState(messages=[],
                                model= None,             
                                next_bot = [],
                                event_list = [],
                                ai_response= ""),
...
if __name__ == "__main__":
    ...
    obj = StoryTeller(scenario = scenario, input_json=json_input)
    obj.build_graph()
    print(obj.invoke())
