...
_GEMINI_MODEL_NAME = "gemini-2.5-flash-lite"  # adjust if you prefer a different Gemini model
...
"""
    ...
    try:
        ...
        # Fallback: treat as invalid but keep original story
        return {
            "valid": False,
            "story": story,
...
def _llm_infer_personality_if_needed(champ_name: str, personality: str | None) -> str:
    ...
    """
    # If user provided a valid personality, keep it exactly
...
def receive_data():
    ...
    # Model aliasing (if you later route by model per champion)
    MODEL_ALIASES = {
    ...
    def normalize_model(m):
        ...
        # Fallback: sanitize into a simple string
    ...
    for c in characters:
        ...
        personality_final = _llm_infer_personality_if_needed(clean_name, raw_personality) or DEFAULT_PERSONALITY
