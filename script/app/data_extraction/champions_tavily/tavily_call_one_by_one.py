...
# Save each character's data immediately after fetching, to avoid losing progress if the script fails.
if __name__ == "__main__":
    out_path = "champion_characters_tavily.json"
    champions = []
    titles = fetch_character_titles_from_category(CATEGORY_TITLE)
    limit = 5  # or set as needed
    ...
    for i, t in enumerate(titles):
        ...
        # Save progress after each champion
        with open(out_path, "w", encoding="utf-8") as f:
            json.dump(champions, f, ensure_ascii=False, indent=2)
        print(f"Saved {len(champions)} entries to {out_path} (latest: {t})")